	...
	if (backlog_count != -1) {
		...
		while(! local_system.IsIntterupted() &&
			...
			if (local_system.IsIntterupted()) {
				...
				} else if (backlog_count == -5) { 
					count = 1; dir = 1; flag = 1 | 4;
				} else if (backlog_count == -6) { 
				...
				if (e) {
					local_system.SetBacklog(-1); 
					continue;
				} else if (l) {
					local_system.SetBacklog(11); 
					continue;
				} else if (r) {
					local_system.SetBacklog(10 + 10000); 
					continue;
				} else if (u && valid_to_old) { 
					valid_to_new = 1;
					if (SkipOldMessage() == -1) valid_to_old = 0;
					is_draw = 1;
				} else if (d && valid_to_new) { 
					valid_to_old = 1;
			...
			if (is_draw) {
				...
				local_system.DrawText_(str); local_system.DrawTextEnd(1);
				is_draw = 0;
			}
...
int SENARIO_BackLog::SetGraphicsInfo(void) {
	...
	int i; for (i=0; i<grp_info_len; i++) {
		if (grp_hash[i] == hash) {
			...
			if (cur_point - grp_info[i] > BACKLOG_LEN/2) continue; 
