...
RegisterSelMacro(70, TLI4(70,71,72,73), WithMask) 
...
RegisterSelMacro(80, TLI4(80,81,82,83), NoMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	int width = sel->x2 - sel->x1 + 1; int height = sel->y2 - sel->y1 + 1;
	if (count == 0) {
...
RegisterSelMacro(170, TLI1(170), WithMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	int width, height;
...
printf("copy line %3d->%3d\n",sel->params[2],copy_line);
	...
	double theta = sel->params[3]*(4*3.141592*2/360); 
	sel->params[3]++;
	int y;
