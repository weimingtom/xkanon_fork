...
class WAKU_PDT {
	...
	WAKU_PDT(void);
	~WAKU_PDT();
	void Init(GdkWindow* win, char* data, char* mask_data, int x, int y, int width); 
	void Draw(GdkImage* d, int x, int y, int width, int height); 
	void MakeImage(GdkWindow* win,  
		char* data, char* mask_data, int x, int y, int width, int height, int src_width,
		GdkImage*& image, char*& mask);
	void CopyImage(GdkImage* d, GdkImage* s, char* mask, int x, int y);  
};
...
void WAKU_PDT::MakeImage(GdkWindow* win,  
...
void WAKU_PDT::CopyImage(GdkImage* d, GdkImage* s, char* mask, int x, int y) {
	...
	int bypp = (s->bpl)/src_width; 
	
	dest_pt += bypp*x + dest_bpl*y; 
...
int SYSTEM_PDT_IMAGE::Init(GdkWindow* window, AyuSys& local_system) {
	...
	InitPixmaps(window, data, width, height, local_system); 
	delete conv;
	return true;
}
...
trans=0;
	...
	if (mx2<0) mx2=0;
	if (my2<0) my2=0;
	if (mx1+mx2 >= width || my1+my2 >= height) return; 
