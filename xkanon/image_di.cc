...
void SetImage(GdkImage* image, unsigned char* data) {
	...
	if (vis->type != GDK_VISUAL_TRUE_COLOR &&
		...
		fprintf(stderr, "Warning in SetImage() in file image_di.cc : "
			"SetImage() only support visual type GDK_VISUAL_TRUE_COLOR"
			" or GDK_VISUAL_DIRECT_COLOR, 15/16/24/32 bpp "
			", please use xdpyinfo to check visual type when X server starting\n");
		return;
	}
