...
void AyuWindow::Draw(void) {
	...
	if (not_synced_flag == 2) DisplaySync(); 
...
int PartWindow::WindowID(void) {
	...
	if (is_realized) return (int)GDK_WINDOW_HWND(wid->window);
	else return -1;
}
...
gint AyuWindow::button_event_impl(GdkEventButton* event) {
	if (event->button == 1) {
		if (event->type == GDK_BUTTON_RELEASE) {
			...
			if (local_system.config->GetParaInt("#MSGBK_MOD")) { 
...
printf("%d / %d\n",n/1000,m/1000);
		...
		case GDK_Home:
		case GDK_KP_Home:
			local_system.SetBacklog(-4); 
			break;
		case GDK_End:
		case GDK_KP_End:
			local_system.StartTextSkipMode(-2); 
			break;
		case GDK_Prior:
		case GDK_KP_Prior:
		/* case GDK_Page_Up: */ 
...
void AyuSys::WaitNextEvent(void) {
	CallProcessMessages();
	if (main_window) gtk_main_iteration(); 
}
...
int AyuWindow::GetWindowID(void) {
	...
	return (int)GDK_WINDOW_HWND(main->window);
}
