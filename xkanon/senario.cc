			...
			{ 
				...
				clicked:
					...
					if (local_system.TextFastMode() == AyuSys::TF_AUTO
						&& subcmd != 4
						&& timer &&  local_system.getTime(timer) > 200) break; 
					if (local_system.TextFastMode() == AyuSys::TF_AUTO
						&& subcmd == 4
						&& timer &&  local_system.getTime(timer) > 500) break; 
...
/* @@@ */
	...
	switch (subcmd) {
		case 0x01:		
			x = Scn_ReadValue(); y = Scn_ReadValue();		
			ex = Scn_ReadValue(); ey = Scn_ReadValue();		
			r = Scn_ReadValue(); g = Scn_ReadValue(); b = Scn_ReadValue();		
			br = Scn_ReadValue(); bg = Scn_ReadValue(); bb = Scn_ReadValue();	
			cmd = 0;
			break;
		case 0x02:		
			idx = Scn_ReadValue();
...
//			Sys_NameInputDlg(nametitle[0], nametitle[1], nameindex[0], nameindex[1]);
	...
	return TRUE;
			...
			if (NextChar() == 0x01) {
				...
				info.SetInfo("NAME", (const char*)macro.GetMacro(arg2), arg2);
		...
		case 0x6a: {
			...
			for (i=0; i<arg; i++) {
				...
				local_system.CopyBuffer(0, 0, local_system.DefaultScreenWidth()-1, imheight+hspace-1, reserve_pdt,
					0, 0, draw_pdt, 0);
				int dest_x = (alignment == 1) ? (local_system.DefaultScreenWidth()-imwidth)/2 : 
						(alignment == 3) ? (local_system.DefaultScreenWidth()-imwidth) : 
						0; 
