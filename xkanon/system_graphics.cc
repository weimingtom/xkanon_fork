...
int AyuSys::IsPDTUsed(int pdt_num) {
	if (pdt_num <= 0 || pdt_num >= PDT_BUFFER_DEAL) return 0; 
	if (pdt_image[pdt_num] == 0) return 0; 
	return 1; 
}
...
void AyuSys::DrawPDTBuffer(int pdt_number, SEL_STRUCT* sel) {
	...
	if (pdt_number == 0 && sel->sel_no != 200) { 
		CallUpdateFunc();
	...
	if (x2 < 0 || y2 < 0 || x1 >= pdt_buffer[pdt_number]->width || y1 >= pdt_buffer[pdt_number]->height) return; 
	if (x1 < 0) { x3 -= x1; x1=0; }
	if (y1 < 0) { y3 -= y1; y1=0; }
	if (x3 < 0) { x1 -= x3; x3=0; }
	if (y3 < 0) { y1 -= y3; y3=0; }
	...
	if (sel->kasane && (sel->arg1 != 0 && sel->arg1 != 0xff) && (
		(sel->sel_no >= 15 && sel->sel_no <= 18) || 
		(sel->sel_no >= 80 && sel->sel_no <= 83) || 
		(sel->sel_no >= 160&& sel->sel_no <= 163)   
		)) {
