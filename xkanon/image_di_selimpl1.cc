...
RegisterSelMacro(4, TLI2(4,5), WithMask) 
...
RegisterSelMacro(10, TLI4(10,11,12,13), WithMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	int end_flag = 0;
...
RegisterSelMacro(15, TLI2(15,16), NoMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	if (count == 0) {
		sel->params[0] = 0;
		return 1;
	...
	int all_height2 = dest.height; 
	if (scroll_height < 0) {
		scroll_dir = -1;
		scroll_height = -scroll_height;
	}
	if (sel->sel_no == 16) scroll_dir = -scroll_dir; 
...
RegisterSelMacro(17, TLI2(17,18), NoMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	if (count == 0) {
		sel->params[0] = 0;
		return 1;
	...
	int all_width2 = dest.width; 
	if (scroll_width < 0) {
		scroll_dir = -1;
		scroll_width = -scroll_width;
	}
	if (sel->sel_no == 18) scroll_dir = -scroll_dir; 
...
RegisterSelMacro(20, TLI4(20,21,22,23), NoMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	int end_flag = 0;
	count++; 
...
RegisterSelMacro(25, TLI4(25,26,40,41), WithMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	int width = sel->x2-sel->x1+1;
	int height = sel->y2-sel->y1+1;
	if (count == 0) {
...
RegisterSelMacro(35, TLI2(35,36), WithMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	int draw_width = sel->arg4;
	int draw_height = sel->arg5;
	if (draw_width <= 0) draw_width = 64; 
...
RegisterSelMacro(45, TLI1(45), WithMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	int width = sel->x2-sel->x1+1;
	int height = sel->y2-sel->y1+1;
	int mid_x = (width+1)/2;
...
RegisterSelMacro(250, TLI1(250), NoMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	if (count == 0) {
		sel->params[0] = 0;
		return 0;
...
RegisterSelMacro(60, TLI4(60,61,62,63), WithMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	if (count == 0) {
		sel->params[0] = 0;
...
RegisterSelMacro(120, TLI10(30,31,100,101,102,103,120,121,122,123), NoMask) 
::Exec(DI_Image& dest, DI_Image& src, char* mask, SEL_STRUCT* sel, int count) {
	if (count == 0) {
		sel->params[0] = 0;
	}
	...
	if (sel_no == 30) {
		if (make_stipple30(sel->params[0], count, width, 1, draw_points) == 0) return 2; 
		sel_no = 120;
	} else if (sel_no == 31) {
		if (make_stipple30(sel->params[0], count, height, 1, draw_points) == 0) return 2; 
		sel_no = 122;
	} else if (sel_no <= 101) {
		...
		if (make_stipple4(sel->params[0], count, width, draw_points) == 0) return 2; 
	} else {
		if (make_stipple4(sel->params[0], count, height, draw_points) == 0) return 2; 
	}
	if (draw_points[0] == -1) return 2; 
