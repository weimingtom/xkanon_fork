...
public:
	...
	int item_deal; 
	char* menu_string; 
	virtual void exec_cmd(int num) = 0; 
	struct ItemShell {
...
AyuWin_FileMenu::AyuWin_FileMenu(AyuWindow* parent, int deal, char* str) {
	...
	for (i=0; i<deal; i++) {
		...
		g_signal_connect(G_OBJECT(subitem), "activate", G_CALLBACK(&exec_cmd_static), shell); 
...
void AyuWin_FileMenu::SetString(char** strlist) {
	...
	if (deal2 > item_deal) deal2 = item_deal; 
